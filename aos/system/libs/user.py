...
class User(object):
    ...
    def get_user_info():
        ...
        if user_product_info:
            try:
                user = User()
                user.email = user_product_info['email'] if 'email' in user_product_info else None
                user.refresh_token = user_product_info['refresh_token'] if 'refresh_token' in user_product_info else ""
                user.access_token = user_product_info['access_token'] if 'access_token' in user_product_info else None
                user.token = user.access_token if user.access_token else user_product_info['token'] if 'token' in user_product_info else None
                user.user_id = user_product_info['id']
                user.user_hash = user_product_info['user_hash'] if 'user_hash' in user_product_info else None
                user.full_name = user_product_info['fullname'] if 'fullname' in user_product_info else user_product_info['full_name'] if 'full_name' in user_product_info else ""
