...
class Util(object):
    ...
    @staticmethod
    def check_internet_connection2(steps = 5, sleeps = 2):
        for j in range(0, steps):
            sleep(sleeps)
            if Util.internet_on2():
                return True
        return False
    ...
    def get_ip_address2() :
        try:
            return str(Util.cmd("hostname -I | awk '{print $1}'")).strip()
        except Exception as ex:
            print "Cannot get address 2 --> " + str(ex)
        return "unknown"
    ...
    @staticmethod
    def check_invalid_data(data_string, key_array=[]):
        if (len(key_array) == 0):
            key_array = ['action', 'verify_code', 'address_long', 'address_lat', 'time_zone', 'user_id', 'wpa', 'ssid', 'product_name', 'platform', 'token']
        result = Util.Result()
        try:
