...
class Ability(object):
    ...
    def list(token=None, product_id=None):
        try:
            ...
            if token and product_id:
                response = RequestApi(token).get_json(__get_ability_list__, query_parameters={"product_id": product_id, "platform": DEVICE_TYPE})
                if response:
    ...
    def get_all_ability_by_product_id(token=None, product_id=None):
        ...
        if token and product_id:
            return RequestApi(token).get_json(__get_ability_list__, query_parameters={"product_id": product_id, "platform": DEVICE_TYPE})
    ...
    def get_all_ability(token=None):
        ...
        user = User.get_user_info()
        if user:
            return RequestApi(user.token).get_json(__get_ability_list__, query_parameters={"platform": DEVICE_TYPE})
