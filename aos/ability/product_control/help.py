...
class Help(object):
    ...
    def open_waiting_update_ability(text, message, percentage):
        ...
        s = {"source": "", "type": "phone_control", "data": data, "protocol": ""}
        send_json(s)
    ...
    def get_os_version():
        ...
        try:
            ...
            import os
            os_version = CURRENT_PATH + 'data/system_version.json'
            if os.path.isfile(os_version):
                from aos.system.libs.util import Util
                version = Util.read_file(os_version)['version']
        except Exception as e:
            print (str(e))
    ...
    def up_os_version():
        try:
            os_version = Help.get_os_version_api()
            if os_version:
                from aos.system.configs.channel import CURRENT_PATH
                import os
                from aos.system.libs.util import Util
                os_version_path = CURRENT_PATH + 'data/system_version.json'
                Util.write_file(os_version_path, {"version": os_version})
        except Exception as e:
            print (str(e))
    ...
    def get_os_version_api():
        try:
            ...
            api_endpoint = 'system-version?platform=MINER'
            try:
