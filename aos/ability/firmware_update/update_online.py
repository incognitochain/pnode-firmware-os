...
FIRMWARE_UPDATE__TMP_PATH_UNZIP = FIRMWARE_UPDATE__TMP_PATH + "draf"
...
class UpdateOnline(object):
    ...
    def unzip(self):
        try:
            ...
            if os.path.isdir(FIRMWARE_UPDATE__TMP_PATH_UNZIP):
                shutil.rmtree(FIRMWARE_UPDATE__TMP_PATH_UNZIP)

            os.mkdir(FIRMWARE_UPDATE__TMP_PATH_UNZIP)

            # giai nen toi thu muc draf truoc:
            with zipfile.ZipFile(self.zip_file, "r") as z:
                print ("extracting to >> ", FIRMWARE_UPDATE__TMP_PATH_UNZIP)
                z.extractall(FIRMWARE_UPDATE__TMP_PATH_UNZIP)

                Util.cmd('sync')

                z.close()

            # move ve thu muc chinh de chuan bi reboot:
            cmd = 'mv %s %s' % (FIRMWARE_UPDATE__TMP_PATH_UNZIP + "/system", FIRMWARE_UPDATE__TMP_PATH)
            Util.cmd(cmd)
        ...
        if self.silent is False:
            ...
            try:
                if DEVICE_TYPE == "SMART_DESK_3":
                    ...
                    if percentage > -1:
                        data = {"action": "update_firmware_status", "from": "product_control",
                                "data": {"status": 1, "message": "", "data": {
                                    "title": "Updating OS...",
                                    "desc": str("%.0f" % percentage) + '%',
                                    "progress": percentage}}}
